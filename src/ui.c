    ...
    char found_names[][32], char found_labels[][64], char *result1, int result1_size, char *result2, int result2_size)
{
    ...
    if (selected_name && selected_name_size > 0)
        selected_name[0] = 0;
    ...
    if (count <= 0) {
        ...
        if (match >= 0) {
            ...
            if (selected_name)
                copy_config_text(selected_name, selected_name_size, found_names[match]);
            ...
            if (cls == IDCMP_CLOSEWINDOW) {
                ...
                if (inside(mx, my, 32, 84, 96, 100)) {
                    ...
                    if (selected_name[0]) {
                        ...
                        copy_config_text(app->data.location, sizeof(app->data.location), selected_name);
                        if (app->win)
                            W13_DrawAll(app);
                    } else if (loc_buf[0]) {
